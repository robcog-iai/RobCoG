...
void ABP_ParentActor::EndPlay(const EEndPlayReason::Type reason) {
	...
	FString FinalString = TEXT("SourceContainer,DestinationContainer,PouringTime,RotationSpeed,MaximumAngle,SourcePosX,SourcePosY,SourcePosZ,DestPosX,DestPosY,DestPosZ,NumberOfParticles,RotationInDegreePerSecond,IsPouringFailed");
	...
	if (TotalDataRaws > 0) {
		...
		FString FirstDataRaw = DataRaws[0] + TEXT(",") + FString::SanitizeFloat((turnedDegrees / timeElapsed)) + TEXT(",") + PourinFailedStr;
