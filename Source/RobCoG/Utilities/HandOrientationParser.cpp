...
void HandOrientationParser::GetHandOrientationsForGraspType(FHandOrientation & InitialHandOrientation, FHandOrientation & ClosedHandOrietation, const FString ConfigPath)
{
	ReadGraspTypeIni(InitialHandOrientation, ClosedHandOrietation, ConfigPath);
}
...
void HandOrientationParser::ReadGraspTypeIni(FHandOrientation & InitialHandOrientation, FHandOrientation & ClosedHandOrientation, const FString ConfigPath)
{
	if (!ConfigFileHandler.IsValid()) return;
	...
	bool bSuccess = ConfigFileHandler->GetRotator(*InitSection, TEXT("ThumbDistalOrientation"), InitialHandOrientation.ThumbOrientation.DistalOrientation.Orientation, ConfigPath);
	if (!bSuccess) return;
	bSuccess = ConfigFileHandler->GetRotator(*InitSection, TEXT("ThumbItermediateOrientation"), InitialHandOrientation.ThumbOrientation.IntermediateOrientation.Orientation, ConfigPath);
	if (!bSuccess) return;
	bSuccess = ConfigFileHandler->GetRotator(*InitSection, TEXT("ThumbProximalOrientation"), InitialHandOrientation.ThumbOrientation.ProximalOrientation.Orientation, ConfigPath);
	if (!bSuccess) return;
	bSuccess = ConfigFileHandler->GetRotator(*InitSection, TEXT("ThumbMetacarpalOrientation"), InitialHandOrientation.ThumbOrientation.MetacarpalOrientation.Orientation, ConfigPath);
	if (!bSuccess) return;
	
	bSuccess = ConfigFileHandler->GetRotator(*InitSection, TEXT("IndexDistalOrientation"), InitialHandOrientation.IndexOrientation.DistalOrientation.Orientation, ConfigPath);
	if (!bSuccess) return;
	bSuccess = ConfigFileHandler->GetRotator(*InitSection, TEXT("IndexItermediateOrientation"), InitialHandOrientation.IndexOrientation.IntermediateOrientation.Orientation, ConfigPath);
	if (!bSuccess) return;
	bSuccess = ConfigFileHandler->GetRotator(*InitSection, TEXT("IndexProximalOrientation"), InitialHandOrientation.IndexOrientation.ProximalOrientation.Orientation, ConfigPath);
	if (!bSuccess) return;
	bSuccess = ConfigFileHandler->GetRotator(*InitSection, TEXT("IndexMetacarpalOrientation"), InitialHandOrientation.IndexOrientation.MetacarpalOrientation.Orientation, ConfigPath);
	if (!bSuccess) return;
	
	bSuccess = ConfigFileHandler->GetRotator(*InitSection, TEXT("MiddleDistalOrientation"), InitialHandOrientation.MiddleOrientation.DistalOrientation.Orientation, ConfigPath);
	if (!bSuccess) return;
	bSuccess = ConfigFileHandler->GetRotator(*InitSection, TEXT("MiddleItermediateOrientation"), InitialHandOrientation.MiddleOrientation.IntermediateOrientation.Orientation, ConfigPath);
	if (!bSuccess) return;
	bSuccess = ConfigFileHandler->GetRotator(*InitSection, TEXT("MiddleProximalOrientation"), InitialHandOrientation.MiddleOrientation.ProximalOrientation.Orientation, ConfigPath);
	if (!bSuccess) return;
	bSuccess = ConfigFileHandler->GetRotator(*InitSection, TEXT("MiddleMetacarpalOrientation"), InitialHandOrientation.MiddleOrientation.MetacarpalOrientation.Orientation, ConfigPath);
	if (!bSuccess) return;
	
	bSuccess = ConfigFileHandler->GetRotator(*InitSection, TEXT("RingDistalOrientation"), InitialHandOrientation.RingOrientation.DistalOrientation.Orientation, ConfigPath);
	if (!bSuccess) return;
	bSuccess = ConfigFileHandler->GetRotator(*InitSection, TEXT("RingItermediateOrientation"), InitialHandOrientation.RingOrientation.IntermediateOrientation.Orientation, ConfigPath);
	if (!bSuccess) return;
	bSuccess = ConfigFileHandler->GetRotator(*InitSection, TEXT("RingProximalOrientation"), InitialHandOrientation.RingOrientation.ProximalOrientation.Orientation, ConfigPath);
	if (!bSuccess) return;
	bSuccess = ConfigFileHandler->GetRotator(*InitSection, TEXT("RingMetacarpalOrientation"), InitialHandOrientation.RingOrientation.MetacarpalOrientation.Orientation, ConfigPath);
	if (!bSuccess) return;
	
	bSuccess = ConfigFileHandler->GetRotator(*InitSection, TEXT("PinkyDistalOrientation"), InitialHandOrientation.PinkyOrientation.DistalOrientation.Orientation, ConfigPath);
	if (!bSuccess) return;
	bSuccess = ConfigFileHandler->GetRotator(*InitSection, TEXT("PinkyItermediateOrientation"), InitialHandOrientation.PinkyOrientation.IntermediateOrientation.Orientation, ConfigPath);
	if (!bSuccess) return;
	bSuccess = ConfigFileHandler->GetRotator(*InitSection, TEXT("PinkyProximalOrientation"), InitialHandOrientation.PinkyOrientation.ProximalOrientation.Orientation, ConfigPath);
	if (!bSuccess) return;
	bSuccess = ConfigFileHandler->GetRotator(*InitSection, TEXT("PinkyMetacarpalOrientation"), InitialHandOrientation.PinkyOrientation.MetacarpalOrientation.Orientation, ConfigPath);
	if (!bSuccess) return;
	
	bSuccess = ConfigFileHandler->GetRotator(*ClosedSection, TEXT("ThumbDistalOrientation"), ClosedHandOrientation.ThumbOrientation.DistalOrientation.Orientation, ConfigPath);
	if (!bSuccess) return;
	bSuccess = ConfigFileHandler->GetRotator(*ClosedSection, TEXT("ThumbItermediateOrientation"), ClosedHandOrientation.ThumbOrientation.IntermediateOrientation.Orientation, ConfigPath);
	if (!bSuccess) return;
	bSuccess = ConfigFileHandler->GetRotator(*ClosedSection, TEXT("ThumbProximalOrientation"), ClosedHandOrientation.ThumbOrientation.ProximalOrientation.Orientation, ConfigPath);
	if (!bSuccess) return;
	bSuccess = ConfigFileHandler->GetRotator(*ClosedSection, TEXT("ThumbMetacarpalOrientation"), ClosedHandOrientation.ThumbOrientation.MetacarpalOrientation.Orientation, ConfigPath);
	if (!bSuccess) return;
	
	bSuccess = ConfigFileHandler->GetRotator(*ClosedSection, TEXT("IndexDistalOrientation"), ClosedHandOrientation.IndexOrientation.DistalOrientation.Orientation, ConfigPath);
	if (!bSuccess) return;
	bSuccess = ConfigFileHandler->GetRotator(*ClosedSection, TEXT("IndexItermediateOrientation"), ClosedHandOrientation.IndexOrientation.IntermediateOrientation.Orientation, ConfigPath);
	if (!bSuccess) return;
	bSuccess = ConfigFileHandler->GetRotator(*ClosedSection, TEXT("IndexProximalOrientation"), ClosedHandOrientation.IndexOrientation.ProximalOrientation.Orientation, ConfigPath);
	if (!bSuccess) return;
	bSuccess = ConfigFileHandler->GetRotator(*ClosedSection, TEXT("IndexMetacarpalOrientation"), ClosedHandOrientation.IndexOrientation.MetacarpalOrientation.Orientation, ConfigPath);
	if (!bSuccess) return;
	
	bSuccess = ConfigFileHandler->GetRotator(*ClosedSection, TEXT("MiddleDistalOrientation"), ClosedHandOrientation.MiddleOrientation.DistalOrientation.Orientation, ConfigPath);
	if (!bSuccess) return;
	bSuccess = ConfigFileHandler->GetRotator(*ClosedSection, TEXT("MiddleItermediateOrientation"), ClosedHandOrientation.MiddleOrientation.IntermediateOrientation.Orientation, ConfigPath);
	if (!bSuccess) return;
	bSuccess = ConfigFileHandler->GetRotator(*ClosedSection, TEXT("MiddleProximalOrientation"), ClosedHandOrientation.MiddleOrientation.ProximalOrientation.Orientation, ConfigPath);
	if (!bSuccess) return;
	bSuccess = ConfigFileHandler->GetRotator(*ClosedSection, TEXT("MiddleMetacarpalOrientation"), ClosedHandOrientation.MiddleOrientation.MetacarpalOrientation.Orientation, ConfigPath);
	if (!bSuccess) return;
	
	bSuccess = ConfigFileHandler->GetRotator(*ClosedSection, TEXT("RingDistalOrientation"), ClosedHandOrientation.RingOrientation.DistalOrientation.Orientation, ConfigPath);
	if (!bSuccess) return;
	bSuccess = ConfigFileHandler->GetRotator(*ClosedSection, TEXT("RingItermediateOrientation"), ClosedHandOrientation.RingOrientation.IntermediateOrientation.Orientation, ConfigPath);
	if (!bSuccess) return;
	bSuccess = ConfigFileHandler->GetRotator(*ClosedSection, TEXT("RingProximalOrientation"), ClosedHandOrientation.RingOrientation.ProximalOrientation.Orientation, ConfigPath);
	if (!bSuccess) return;
	bSuccess = ConfigFileHandler->GetRotator(*ClosedSection, TEXT("RingMetacarpalOrientation"), ClosedHandOrientation.RingOrientation.MetacarpalOrientation.Orientation, ConfigPath);
	if (!bSuccess) return;
	
	bSuccess = ConfigFileHandler->GetRotator(*ClosedSection, TEXT("PinkyDistalOrientation"), ClosedHandOrientation.PinkyOrientation.DistalOrientation.Orientation, ConfigPath);
	if (!bSuccess) return;
	bSuccess = ConfigFileHandler->GetRotator(*ClosedSection, TEXT("PinkyItermediateOrientation"), ClosedHandOrientation.PinkyOrientation.IntermediateOrientation.Orientation, ConfigPath);
	if (!bSuccess) return;
	bSuccess = ConfigFileHandler->GetRotator(*ClosedSection, TEXT("PinkyProximalOrientation"), ClosedHandOrientation.PinkyOrientation.ProximalOrientation.Orientation, ConfigPath);
	if (!bSuccess) return;
	bSuccess = ConfigFileHandler->GetRotator(*ClosedSection, TEXT("PinkyMetacarpalOrientation"), ClosedHandOrientation.PinkyOrientation.MetacarpalOrientation.Orientation, ConfigPath);
	if (!bSuccess) return;
}